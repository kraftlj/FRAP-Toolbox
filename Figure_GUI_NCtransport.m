...
columnformat = [repmat({'numeric'},1,3)];
...
%% Define the Callback functions
    ...
    function data=plotbutton_callback(hObj,event,data)
        ...
        usrinputs(3,1:2)=get(t2,'Data');
        usrinputs{4,1}=get(Avgh,'Value');
        ...
        % Fit with the NC transport model
                [data avg]=NCtransportModel(data,basicinput,usrinputs,val);
        ...
        if usrinputs{4,1}==1
            for index1=1:length(val)
                temp{val(index1),1}=data(val(index1)).Knc;
                temp{val(index1),2}=data(val(index1)).k;
                temp{val(index1),3}=data(val(index1)).SS;
            end
        end
        temp{length([basicinput(:,1)])+1,1}=avg.Knc;
        temp{length([basicinput(:,1)])+1,2}=avg.k;
        temp{length([basicinput(:,1)])+1,3}=avg.SS;
    ...
    function data=savebutton_callback(hObj,event,data)
        ...
        for j=1:length(dataout)
            ...
            a{count+2}=sprintf([basicinput{j,1},'\t FRAP Cytoplasm\t',repmat('%f\t',1,length(dataout(j).cytoplasm)-1),'%f\r\n'],dataout(j).cytoplasm);
            a{count+3}=sprintf([basicinput{j,1},'\t Normalized FRAP\t',repmat('%f\t',1,length(dataout(j).frap)-1),'%f\r\n'],dataout(j).frap);
            a{count+4}=sprintf([basicinput{j,1},'\t Fit Time\t',repmat('%f\t',1,length(dataout(j).t)-1),'%f\r\n'],dataout(j).t);
            a{count+5}=sprintf([basicinput{j,1},'\t FRAP Fit\t',repmat('%f\t',1,length(dataout(j).frapfit)-1),'%f\r\n'],dataout(j).frapfit);
            a{count+6}=sprintf([basicinput{j,1},'\t Fit Residuals\t',repmat('%f\t',1,length(dataout(j).frapres)-1),'%f\r\n'],dataout(j).frapres);
            count=count+7;
        end
        j=1;
