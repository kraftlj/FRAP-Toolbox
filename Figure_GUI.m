...
%% Define the Callback functions
    ...
    function data=plotbutton_callback(hObj,event,data)
        ...
        usrinputs(6:8,1:2)=get(t2,'Data');
        usrinputs{9,1}=get(Avgh,'Value');
        ...
        %------------------------------------------------------------------
        switch basicinput{1,2}
            case 1 % Fit with diffusion model from Kang et al, Biophys J, 2008.
                [data avg]=DiffusionModel_2(data,basicinput,usrinputs,val);
            case 2 % Fit with one component reaction model
                [data avg]=Reaction1Model(data,basicinput,usrinputs,val);
            case 3 % Fit with two component reaction model
                [data avg]=Reaction2Model(data,basicinput,usrinputs,val);
        end
        ...
        if usrinputs{9,1}==1
            for index1=1:length(val)
                ...
                temp{val(index1),2}=data(val(index1)).ceq;
                temp{val(index1),3}=data(val(index1)).MF;
                temp{val(index1),4}=data(val(index1)).SS;
            end
        end
        temp{length([basicinput(:,1)])+1,1}=avg.k;
        temp{length([basicinput(:,1)])+1,2}=avg.ceq;
        temp{length([basicinput(:,1)])+1,3}=avg.MF;
