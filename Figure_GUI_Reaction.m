...
UserInputsh = uipanel('Title','User Inputs','Units',...
    'normalized','Position',[0.0169    0.0169    0.33    0.97],'BackgroundColor',...
...
FitOutputsh = uipanel('Title','Fit Outputs','Units',...
    'normalized','Position',[0.36    0.0169    0.53    0.97],'BackgroundColor',...
    get(GUIfigureh,'color'),'FontSize',14,'FontWeight','bold');

columnname = {'a', 'b', 'c', 'SS'}; % This is the table where the optimized parameters will output for visual inspection
...
%% Define the Callback functions
    ...
    function data=plotbutton_callback(hObj,event,data)
        try
            ...
            usrinputs(5:6,1:2)=get(t2,'Data'); % Fetch data exclusion parameters.
            usrinputs{7,1}=get(Avgh,'Value'); % Fetches the choice about fitting the average data
                ...
                switch usrinputs{4,4}
                    ...
                    case 'Fixed' %If you don't want to correct for photodecay select 'Fixed' in the initial, lower, and upper bound box, and enter 0 as the initial value.
                        for index1=1:length(val)
                            ...
                            data(val(index1)).correctfrap=data(val(index1)).normfrap./exp(-usrinputs{4,1}*t(1,:));
                            decayrate=usrinputs{4,1};
                        end
                end
            end
...
%% Saving the optimized parameters, FRAP curves, and FRAP fits to a tab delimited text file
    function data=savebutton_callback(hObj,event,data)
        try
            ...
            header={'FileNames','a','b','c','SS'};
            fid = fopen(fullfile(FileLocation,[answer{:},'_Reaction_Fit_Parameters.txt']),'w');
            ...
            str=[a{:}];
            fid = fopen(fullfile(FileLocation,[answer{:},'_Reaction_FRAP_datasets.txt']),'w');
            fprintf(fid,str);
            fclose(fid);
            clearvars a str
        catch errObj
