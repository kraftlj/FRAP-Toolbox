...
function [nucleusroimask cytoplasmroimask]=ROIinitialization_NCtransport(img,basicinput)
...
h=msgbox({'Define the adjacent cytoplasm region using the mouse.  Right click and press create mask when finished.'});
movegui(h,'northwest');
figure
cytoplasmroimask = roipoly(imadjust(img,stretchlim(img,[0,1]))); % Create a mask using the user defined ROI
h2=gcf;
close(h);
close(h2);
