...
function [r, pbp]=InitialConditions_FRAP_FRET(basicinput,data)
...
for index1=1:basicinput{1,8} % Load prebleach images into variable img
    index2=basicinput{1,6}*2-basicinput{1,8}*2:2:(basicinput{1,6}-1)*2;
    img(:,:,index1)=double(data{1,1}{index2(index1),1});
end
img2=mean(img,3); % Load mean prebleach image into variable img2
pbi=double(data{1,1}{basicinput{1,6}*2,1}); % Load postbleach image into variable pbi
