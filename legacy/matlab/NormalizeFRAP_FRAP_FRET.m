...
function [normfrap, correctMF, normadjacent, normfret]=NormalizeFRAP_FRAP_FRET(frap,cell,basicinput,adjacent,fret,cellfret)
...
if basicinput{1,4}==2 % IF normalizing by whole cell intensity
    ...
    normfret=fret./cellfret;
    normfret=normfret./mean(normfret(basicinput{1,6}-1-basicinput{1,8}+1:basicinput{1,6}-1));
    ...
    normfret=fret./mean(fret(basicinput{1,6}-1-basicinput{1,8}+1:basicinput{1,6}-1));
